...
const About = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero Section */}
      <section className="relative pt-32 pb-32 px-6 overflow-hidden" style={{ backgroundColor: '#F5F0E8' }}>
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-96 h-96 bg-violeta rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-0 left-0 w-96 h-96 bg-sage rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
      ...
      <section className="relative py-32 px-6 overflow-hidden bg-white">
        <div className="container mx-auto max-w-4xl relative z-10">
          ...
          <div className="space-y-8 font-quicksand text-lg leading-relaxed" style={{ color: '#3D3935' }}>
            <p>
              ...
              >
                <CardContent className="p-6 text-center">
                  <div 
                    ...
                    style={{ backgroundColor: `${index === 0 ? '#6B4FA3' : index === 1 ? '#4A5F8C' : '#8BA888'}20` }}
                  >
                    <direction.icon 
                      className="w-8 h-8" 
                      style={{ color: index === 0 ? '#6B4FA3' : index === 1 ? '#4A5F8C' : '#8BA888' }}
                    />
                  </div>
                  <h3 className="text-xl font-cormorant font-bold mb-2" style={{ color: '#3D3935' }}>
                    {direction.title}
                  </h3>
                  <p className="text-sm font-quicksand" style={{ color: '#8B8680' }}>
            ...
            {values.map((value, index) => (
              ...
              >
                <div 
                  ...
                  style={{ backgroundColor: '#6B4FA320' }}
                >
                  <value.icon className="w-8 h-8 text-violeta" />
                </div>
                <h3 className="text-2xl font-cormorant font-semibold" style={{ color: '#3D3935' }}>
                  {value.title}
                </h3>
                <p className="font-quicksand leading-relaxed" style={{ color: '#8B8680' }}>
                  {value.description}
                </p>
              </div>
            ))}
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="space-y-6">
            {beliefs.map((belief, index) => (
              <div 
                key={index}
                className="flex items-start gap-4 p-6 rounded-xl bg-gradient-to-r from-transparent to-transparent hover:from-sage/10 hover:to-transparent transition-all duration-300"
              >
                <div className="mt-1">
                ...
                <p className="text-lg font-quicksand" style={{ color: '#3D3935' }}>
                  {belief}
                </p>
              </div>
            ))}
      ...
      {/* Fun Facts */}
      <section className="py-20 px-6" style={{ backgroundColor: '#E8DFD3' }}>
        <div className="container mx-auto max-w-4xl">
          <h2 className="text-5xl font-cormorant font-bold text-center mb-4 text-violeta">
            Random Things About Me
          </h2>
          <p className="text-center font-lavishly text-3xl text-azul mb-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {funFacts.map((fact, index) => (
              <div 
                key={index}
                className="flex items-start gap-4 p-6 bg-white rounded-xl hover-lift"
              >
                <div 
                ...
                <p className="font-quicksand text-lg pt-2" style={{ color: '#3D3935' }}>
                  {fact.text}
                </p>
              </div>
            ))}
      ...
      <section className="py-20 px-6 bg-white">
        <div className="container mx-auto max-w-6xl">
          ...
          <p className="text-center font-quicksand mb-16" style={{ color: '#8B8680' }}>
              ...
              >
                <Card className="h-full hover-lift border-2 border-transparent hover:border-sage transition-all duration-300">
                  <CardContent className="p-6">
                    <h3 className="text-2xl font-cormorant font-semibold mb-2 group-hover:text-sage transition-colors" style={{ color: '#3D3935' }}>
                      {offering.title}
                    </h3>
                    <p className="font-quicksand" style={{ color: '#8B8680' }}>
        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-12 md:p-16 shadow-2xl">
            <div className="text-center space-y-8">
              ...
              <h2 className="text-4xl md:text-5xl font-cormorant font-bold leading-tight" style={{ color: '#3D3935' }}>
              ...
              <p className="text-xl font-quicksand max-w-2xl mx-auto leading-relaxed" style={{ color: '#8B8680' }}>
